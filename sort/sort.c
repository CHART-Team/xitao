...
int merge_task_size = MERGE_TASK_SIZE;
...
{
        if(argc == 0) printf("Usage: ./sort <buffer_size> <sort_size> <merge_size> <insertion_threshold>\n");
        if(argc > 1) sort_buffer_size = atoi(argv[1]);
        if(argc > 2) sort_task_size = atoi(argv[2]);
        if(argc > 3) merge_task_size = atoi(argv[3]);
        if(argc > 4) insertion_thr = atoi(argv[4]);
        ...
        //bots_message("Computing multisort algorithm (n=%d) ", sort_buffer_size);
        cilksort(array, tmp, sort_buffer_size);
...
#elif defined (STATICSORT)

#define BLOCKSIZE (16*131072)

int main ( int argc, char *argv[] )
{
        sort_buffer_size = 4*BLOCKSIZE;
        sort_task_size   = BLOCKSIZE;
        merge_task_size  = BLOCKSIZE;
        insertion_thr    = 20;
        ...
        long quarter = sort_buffer_size / 4;
        ELM *A, *B, *C, *D, *tmpA, *tmpB, *tmpC, *tmpD;
        ...
        seqquick(A, A + quarter - 1);
        seqquick(B, B + quarter - 1);
        seqquick(C, C + quarter - 1);
        seqquick(D, D + sort_buffer_size - 3 * quarter -1 );

//        print_array(64, array);

        // cilkmerge 1
        ELM *split1_m1, *split2_m1;
        long int lowsize_m1;
  
        split1_m1 = (quarter / 2) + A;
        split2_m1 = binsplit(*split1_m1, B, B + quarter-1);
        lowsize_m1 = split1_m1 - A + split2_m1 - B;
   
        /* 
         * directly put the splitting element into
         * the appropriate location
         */
        *(tmp + lowsize_m1 + 1) = *split1_m1;
   
        // parallel merge
        seqmerge(A, split1_m1 - 1, B, split2_m1, tmp);
        seqmerge(split1_m1 + 1, A + quarter -1, split2_m1 + 1, B + quarter -1, tmp+lowsize_m1+2);
        // local barrier


        // cilkmerge 2
        ELM *split1_m2, *split2_m2;
        long int lowsize_m2;
  
        split1_m2 = ((quarter) / 2) + C;
        split2_m2 = binsplit(*split1_m2, D, D + sort_buffer_size - 3 * quarter -1);
        lowsize_m2 = split1_m2 - C + split2_m2 - D;
  
        /* 
         * directly put the splitting element into
         * the appropriate location
         */
        *(tmp + 2*quarter + lowsize_m2 + 1) = *split1_m2;
  
        // parallel merge
        seqmerge(C, split1_m2 - 1, D, split2_m2, tmp + 2*quarter);
        seqmerge(split1_m2 + 1, C + quarter -1, split2_m2 + 1, D + sort_buffer_size - 3 * quarter -1, tmp + 2*quarter + lowsize_m2 + 2);
        // local barrier
 ...
 //       print_array(64, tmp);
        ...
        // next merge all 
        //
        // cilkmerge 3  (half utilization)
        ELM *split1_m3, *split2_m3;
        long int lowsize_m3;
  
        split1_m3 = tmp + quarter;
        split2_m3 = binsplit(*split1_m3, tmp+2*quarter, tmp + sort_buffer_size - 1);
        lowsize_m3 = split1_m3 - tmp + split2_m3 - (tmp + 2*quarter);
   
        /* 
         * directly put the splitting element into
         * the appropriate location
         */
        *(array + lowsize_m3 + 1) = *split1_m3;
   
        // parallel merge
        seqmerge(tmp, split1_m3 - 1, tmp + 2*quarter, split2_m3, A);
        seqmerge(split1_m3 + 1, tmp + 2*quarter -1, split2_m3 + 1, tmp + sort_buffer_size - 1, A+lowsize_m3+2);

        // this are the sequential merges to be parallelized 
        //seqmerge(A, A + quarter -1, B, B + quarter -1, tmp);
        //seqmerge(C, C + quarter -1, D, D + quarter -1, tmp + 2*quarter);

        // seqmerge(tmp, tmp + 2*quarter -1, tmp + 2*quarter, tmp + 4*quarter -1, array);
  //      print_array(64, array);
        if(!sort_verify ( )) {
                printf("Sort Failed!\n"); 
                return 1;
