...
void validate_classifier_10(char *datacfg, char *filename, char *weightfile)
{
    ...
    for(i = 0; i < m; ++i){
        ...
        free(pred);
        if(indexes[0] == class1) avg_acc += 1;
        for(j = 0; j < topk; ++j){
            if(indexes[j] == class1) avg_topk += 1;
        }
...
void predict_classifier(char *datacfg, char *cfgfile, char *weightfile, char *filename, int top, int block_len, int xitao_width)
{
    network *net = load_network(cfgfile, weightfile, 0);
    set_batch_network(net, 1);
    srand(2222222);
    net->block_len=block_len;
    net->xitao_width=xitao_width;
    list *options = read_data_cfg(datacfg);
...
void run_classifier(int argc, char **argv)
{
    ...
    block_len = (argc > 7) ? atoi(argv[7]): 8;
    xitao_width = (argc > 8) ? atoi(argv[8]): 1;

    int layer = layer_s ? atoi(layer_s) : -1;
    predict_classifier(data, cfg, weights, filename, top, block_len, xitao_width);
