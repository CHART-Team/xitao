...
void delta_region_class(float *output, float *delta, int index, int class1, int classes, tree *hier, float scale, int stride, float *avg_cat, int tag)
{
    int i, n;
    if(hier){
        float pred = 1;
        while(class1 >= 0){
            pred *= output[index + stride*class1];
            int g = hier->group[class1];
            int offset = hier->group_offset[g];
            for(i = 0; i < hier->group_size[g]; ++i){
                delta[index + stride*(offset + i)] = scale * (0 - output[index + stride*(offset + i)]);
            }
            delta[index + stride*class1] = scale * (1 - output[index + stride*class1]);

            class1 = hier->parent[class1];
        }
        *avg_cat += pred;
    } else {
        if (delta[index] && tag){
        ...
        for(n = 0; n < classes; ++n){
            delta[index + stride*n] = scale * (((n == class1)?1 : 0) - output[index + stride*n]);
            if(n == class1) *avg_cat += output[index + stride*n];
        }
    }
}
    ...
    for (b = 0; b < l.batch; ++b) {
        if(l.softmax_tree){
            ...
            for(t = 0; t < 30; ++t){
                ...
                if(!truth.x) break;
                int class1 = net.truth[t*(l.coords + 1) + b*l.truths + l.coords];
                float maxp = 0;
                int maxi = 0;
                if(truth.x > 100000 && truth.y > 100000){
                    ...
                    delta_region_class(l.output, l.delta, class_index, class1, l.classes, l.softmax_tree, l.class_scale, l.w*l.h, &avg_cat, !l.softmax);
