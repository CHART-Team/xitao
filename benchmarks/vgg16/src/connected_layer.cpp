...
{
 forward_connected_layer_xitao(l, net);
/*
...
void forward_connected_layer_xitao(layer l, network net)
{

	fill_cpu(l.outputs*l.batch, 0, l.output, 1);
	//CON_TAO1 *tao1 = new CON_TAO1(&l, &net, 10);
	//CON_TAO3 *tao3 = new CON_TAO3(&l, &net, 10);
	//CON_TAO4 *tao4 = new CON_TAO4(&l, &net, 10);
	TAO_barrier *tao_barrier = new TAO_barrier(1);
	//tao1->make_edge(tao3);
	//tao3->make_edge(tao4);
	//tao4->make_edge(tao_barrier);
	//gotao_push(tao1,1);
// MINI TAOS GEMM <3 <3
	int block_length = net.block_len;
        int mini_taos = l.outputs/block_length;
	//cout<< "\nSize = "<<l.outputs<<"  block_length = "<<block_length<< "  mini_taos = "<<mini_taos<<endl;
	CON_TAO3 *t3[mini_taos];
        CON_TAO4 *t4[mini_taos];
	for(int p = 0; p< mini_taos; p++){
		if(p==mini_taos-1)
		block_length= block_length+ (l.outputs%block_length);
                t3[p] = new CON_TAO3(&l, &net, block_length,p, net.xitao_width);
        }
        for(int q = 0; q< mini_taos; q++){
		if(q==mini_taos-1)
                block_length= block_length+ (l.outputs%block_length);
                t4[q] = new CON_TAO4(&l, &net, block_length,q, net.xitao_width);
        }
