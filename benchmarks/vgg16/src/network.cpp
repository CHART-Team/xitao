...
void forward_network_xitao(network *netp)
{
printf("\nLAUNCHING NETWORK  with WIDTH = %d  BLOCK_LENGTH = %d\n",netp->xitao_width, netp->block_len);
        //start GOTAO
	gotao_init();
	gotao_start();
    network net = *netp;
    int i;
//	cout<<"LAUNCHING NETWORK  with WIDTH = "<<net.xitao_width<<" BLOCK_LENGTH = "<<net.block_len<<endl;
    for(i = 0; i < net.n; ++i){
        net.index = i;
        layer l = net.layers[i];
        if(l.delta){
	...
	double wtime1 = omp_get_wtime();
        l.forward(l, net);
	//printf("\nTime in Layer %d is %f seconds\n", i,omp_get_wtime()-wtime1 );
